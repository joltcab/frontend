...
export default function UniversalLogin() {
  ...
  useEffect(() => {
    // If redirected from social auth with token, finalize login
    const params = new URLSearchParams(window.location.search);
    const token = params.get('token');
    if (token) {
      finalizeSocialLogin(token);
    } else {
      checkAuth();
    }
  }, []);
  ...
  const finalizeSocialLogin = async (token) => {
    try {
      joltcab.setToken(token);
      const pendingRole = localStorage.getItem('pendingUserRole');
      if (pendingRole) {
        // Note: backend should set role; we only use it for redirect after login
        localStorage.removeItem('pendingUserRole');
      }
      const user = await joltcab.auth.me();
      if (!user) throw new Error('Unable to fetch user after social login');
      ...
      console.error('❌ Social login finalize error:', error);
              ...
              <div className="mt-6 grid grid-cols-3 gap-3">
                <Button
                  ...
                  onClick={() => {
                    ...
                    joltcab.auth.googleLogin(redirectUri, 'user');
                  }}
                >
                ...
                <Button
                  ...
                  onClick={() => {
                    const redirectUri = window.location.origin + window.location.pathname;
                    joltcab.auth.appleLogin(redirectUri, 'user');
                  }}
                >
                ...
                <Button
                  ...
                  onClick={() => {
                    const redirectUri = window.location.origin + window.location.pathname;
                    joltcab.auth.facebookLogin(redirectUri, 'user');
                  }}
                >
